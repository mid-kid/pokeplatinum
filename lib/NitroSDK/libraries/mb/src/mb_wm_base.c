    ...
    case WM_APIID_START_PARENT:
        {
            ...
            case WM_STATECODE_CHILD_CONNECTED:
                ...
                if ((p_mbi_param->mpStarted == 0) && (!mbc->start_mp_busy))
                {
                    ...
                    errcode = WM_StartMPEx(MBi_ParentCallback, (u16 *)p_mbi_param->recvBuf, p_mbi_param->recvBufSize, (u16 *)p_mbi_param->sendBuf, p_mbi_param->sendBufSize, (u16)(p_mbi_param->contSend ? 0 : 1), 0,   
                                           FALSE, FALSE, 1,     
                                           TRUE 
                        );
    ...
    case WM_APIID_START_CONNECT:
        {
            ...
            case WM_STATECODE_CONNECTED:
                ...
                errcode = WM_StartMPEx(MBi_ChildCallback, (u16 *)p_param->recvBuf, p_param->recvBufSize, (u16 *)p_param->sendBuf, p_param->sendBufSize, (u16)(p_param->contSend ? 0 : 1), 0,    
                                       FALSE, FALSE, 1, 
                                       TRUE     
                    );
                MBi_CheckWmErrcode(WM_APIID_START_MP, errcode);
...
int MBi_SendMP(const void *buf, int len, int pollbmp)
{
    ...
    {
    case MB_MODE_PARENT:
        errcode = MBi_SetMPData(p_mbi_param->callback_ptr, (u16 *)buf, size,    
