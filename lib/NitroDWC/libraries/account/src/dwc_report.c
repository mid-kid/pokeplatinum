...
#include <base/dwc_report.h>
    ...
    static u32 sReportLevel;
        ...
        static void DWCs_GsDebugCallback(GSIDebugCategory theCat, GSIDebugType theType, GSIDebugLevel theLevel, const char * theTokenStr, va_list theParamList) {
        #pragma unused(theLevel)
            DWC_Printf(DWC_REPORTFLAG_GAMESPY, "%d [%s][%s] ", (u32)OS_TicksToMilliSeconds(OS_GetTick()), gGSIDebugCatStrings[theCat], gGSIDebugTypeStrings[theType]);
