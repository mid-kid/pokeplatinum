...
void NNS_G3dGeSendDL(const void * src, u32 szByte) {
    ...
    if (szByte < 256 || GX_DMAID == GX_DMA_NOT_USE) {
        ...
        if (NNS_G3dFlagUseFastDma) {
            MI_SendGXCommandAsyncFast(GX_DMAID, src, szByte, &simpleUnlock_, (void *)&NNS_G3dFlagGXDmaAsync);
        } else {
            MI_SendGXCommandAsync(GX_DMAID, src, szByte, &simpleUnlock_, (void *)&NNS_G3dFlagGXDmaAsync);
        }
    }
}
