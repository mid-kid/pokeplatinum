...
static void G3dDrawInternal_Loop_(NNSG3dRS * pRS) {
    do {
        ...
        NNS_G3D_ASSERTMSG(NNS_G3dFuncSbcTable[NNS_G3D_GET_SBCCMD(*pRS->c)], "SBC command not found / disabled.");

        (*NNS_G3dFuncSbcTable[NNS_G3D_GET_SBCCMD(*pRS->c)])(pRS, NNS_G3D_GET_SBCFLG(*pRS->c));
...
void NNS_G3dDraw(NNSG3dRenderObj * pRenderObj) {
    ...
    NNS_G3D_ASSERTMSG(pRenderObj->resMdl->info.numMat <= NNS_G3D_SIZE_MAT_MAX, "numMaterial exceeds NNS_G3D_SIZE_MAT_MAX");
    NNS_G3D_ASSERTMSG(pRenderObj->resMdl->info.numNode <= NNS_G3D_SIZE_JNT_MAX, "numNode exceeds NNS_G3D_SIZE_JNT_MAX");
    NNS_G3D_ASSERTMSG(pRenderObj->resMdl->info.numShp <= NNS_G3D_SIZE_SHP_MAX, "numShape exceeds NNS_G3D_SIZE_SHP_MAX");
    ...
    if (!(rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW) &&
        ...
        if (!cbFlag) {
            ...
            if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
                NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &arg, G3OP_MTX_RESTORE_NPARAMS);
            }
        }
...
void NNSi_G3dFuncSbc_SHP(NNSG3dRS * rs, u32 opt) {
    ...
    if (!(rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW)) {
        if ((rs->flag & NNS_G3D_RSFLAG_NODE_VISIBLE) &&
            !(rs->flag & NNS_G3D_RSFLAG_MAT_TRANSPARENT)) {
            u32 idxShp = *(rs->c + 1);
            const NNSG3dResShpData * shp = NNS_G3dGetShpDataByIdx(rs->pResShp, idxShp);
...
void NNSi_G3dFuncSbc_NODEDESC(NNSG3dRS * rs, u32 opt) {
    ...
    if (rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCMTXCALC) {
        ...
        if (opt == NNS_G3D_SBCFLG_001 ||
            ...
            if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
                ...
                NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &idxMtxDest, G3OP_MTX_RESTORE_NPARAMS);
            }
        }
        rs->c += cmdLen;
        return;
...

    {
        if (opt == NNS_G3D_SBCFLG_010 ||
            ...
            if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
                NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &idxMtxSrc, G3OP_MTX_RESTORE_NPARAMS);
            }
        }
    }
    ...
    if (!cbFlag) {
        ...
        if (!isUseRecordData) {
            ...
            if (rs->pRenderObj->anmJnt &&
                ...
                const NNSG3dResNodeData * pNd =NNS_G3dGetNodeDataByIdx(rs->pResNodeInfo, idxNode);
    ...
    if (opt == NNS_G3D_SBCFLG_001 ||
        ...
        if (!cbFlag) {
            if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
                u32 idxMtxDest = *(rs->c + 4);
                NNS_G3D_ASSERT(idxMtxDest < 31);
                NNS_G3dGeBufferOP_N(G3OP_MTX_STORE, &idxMtxDest, G3OP_MTX_STORE_NPARAMS);
            }
        }
    }
...
void NNSi_G3dFuncSbc_BB(NNSG3dRS * rs, u32 opt) {
...

    {
        if (opt == NNS_G3D_SBCFLG_010 ||
            ...
            if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
                ...
                NNS_G3D_ASSERT(idxMtxSrc < 31);
                NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &idxMtxSrc, 1);
            }
        }
    }
    ...
    if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD) &&
        ...
        if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_WVP) {
            ...
            MI_CpuSend32(&bbcmd1[1], &reg_G3X_GXFIFO, 2 * sizeof(u32));
            MI_CpuSend32(NNS_G3dGlbGetInvSrtCameraMtx(), &reg_G3X_GXFIFO, G3OP_MTX_LOAD_4x3_NPARAMS * sizeof(u32));

            reg_G3X_GXFIFO = GX_PACK_OP(G3OP_MTX_MULT_4x3, G3OP_MTX_SCALE, G3OP_NOP, G3OP_NOP);
            MI_CpuSend32(&bbcmd1[3], &reg_G3X_GXFIFO, sizeof(MtxFx43) + sizeof(VecFx32));
        } else if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_VP) {
            reg_G3X_GXFIFO = GX_PACK_OP(G3OP_MTX_POP, G3OP_MTX_MODE, G3OP_MTX_LOAD_4x3, G3OP_NOP);
            MI_CpuSend32(&bbcmd1[1], &reg_G3X_GXFIFO, 2 * sizeof(u32));
            MI_CpuSend32(NNS_G3dGlbGetInvCameraMtx(), &reg_G3X_GXFIFO, G3OP_MTX_LOAD_4x3_NPARAMS * sizeof(u32));
...
void NNSi_G3dFuncSbc_POSSCALE(NNSG3dRS * rs, u32 opt) {
    ...
    if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD) &&
        ...
        NNS_G3dGeBufferOP_N(G3OP_MTX_SCALE, (u32 *)&s.x, G3OP_MTX_SCALE_NPARAMS);
    }
