    ...
    void NNS_G3dPrintTexPatAnm(const NNSG3dResTexPatAnm * pPatAnm) {
...

        {
            ...
            for (i = 0; i < numEntries; i++) {
                const NNSG3dResDictTexPatAnmData * pAnmData = NNSi_G3dGetTexPatAnmDataByIdx(pPatAnm, i);

                {
                    const NNSG3dResName * name = NNS_G3dGetResNameByIdx(&pPatAnm->dict, i);
...

                {
                    ...
                    for (j = 0; j < pAnmData->numFV; j++) {
                        const NNSG3dResTexPatAnmFV * pTexPatAnm = NNSi_G3dGetTexPatAnmFVByFVIndex(pPatAnm, i, j);
                        tabPrint_();
                        RES_PRINTF("frame, %d, ", pTexPatAnm->idxFrame);

                        {
                            const NNSG3dResName * name = (const NNSG3dResName *)((u8 *)pPatAnm + pPatAnm->ofsTexName);
                            RES_PRINTF("texture, ");
                            NNS_G3dPrintResName(&name[pTexPatAnm->idTex]);
                        }
                        {
                            const NNSG3dResName * name = (const NNSG3dResName *)((u8 *)pPatAnm + pPatAnm->ofsPlttName);
                            RES_PRINTF(", palette, ");
