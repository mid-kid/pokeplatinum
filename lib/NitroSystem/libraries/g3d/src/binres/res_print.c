...
#include <math.h>
    ...
    void NNS_G3dPrintResName(const NNSG3dResName * resName) {
        u32 i;
    ...
    void NNS_G3dPrintFileHeader(const NNSG3dResFileHeader * header) {
    ...
    void NNS_G3dPrintDataBlockHeader(const NNSG3dResDataBlockHeader * header) {
        const char * kind;
        NNS_G3D_NULL_ASSERT(header);
    ...
    void NNS_G3dPrintDictEntryItem_TEX(u32 idx, const NNSG3dResName * name, const NNSG3dResDictTexData * data) {
        u32 texAddr;
        int width, height;
        u32 fmt;
        u32 pltt0;
        ...
        RES_PRINTF(", %d, %d\n", data->extraParam & 0x3ff, (data->extraParam >> 11) & 0x3ff);
    }

    void NNS_G3dPrintDictEntryItem_PLTT(u32 idx, const NNSG3dResName * name, const NNSG3dResDictPlttData * data) {
        NNS_G3D_NULL_ASSERT(name);
        NNS_G3D_NULL_ASSERT(data);
    ...
    void NNS_G3dPrintTex(const NNSG3dResTex * texImage) {
        ...
        for (idx = 0; idx < numEntry; ++idx) {
            const NNSG3dResDictTexData * data = (const NNSG3dResDictTexData *) NNS_G3dGetResDataByIdx(dict, idx);
        ...
        if (0 != numEntryNm) {
            {
                ...
                tabPlus_();
                {
                    ...
                    for (i = 0; i < numEntry; ++i) {
                        ...
                        const NNSG3dResDictTexData * data = (const NNSG3dResDictTexData *)NNS_G3dGetResDataByIdx(dict, i);
    ...
    void NNS_G3dPrintMdlInfo(const NNSG3dResMdlInfo * info) {
    ...
    void NNS_G3dPrintNodeSRTInfo(const NNSG3dResNodeInfo * info, u32 idx) {
        ...
        {
            u32 mtxStackID = ((u32)srt->flag & NNS_G3D_SRTFLAG_IDXMTXSTACK_MASK) >> NNS_G3D_SRTFLAG_IDXMTXSTACK_SHIFT;
            if (mtxStackID != 31) {
                RES_PRINTF(", stackID = %d", mtxStackID);
            } else {
                RES_PRINTF(", stackID = N/A");
    ...
    void NNS_G3dPrintNodeInfo(const NNSG3dResNodeInfo * info) {
        NNS_G3D_NULL_ASSERT(info);
    ...
    static const NNSG3dResName * getNodeName_(u32 idx, const NNSG3dResMdl * mdl) {
        NNS_G3D_NULL_ASSERT(mdl);
        return NNS_G3dGetResNameByIdx(&mdl->nodeInfo.dict, idx);
    }

    static const NNSG3dResName * getMatName_(u32 idx, const NNSG3dResMdl * mdl) {
    ...
    static const NNSG3dResName * getShpName_(u32 idx, const NNSG3dResMdl * mdl) {
    ...
    static void printOpNode_(u32 idx, const NNSG3dResMdl * mdl) {
        NNS_G3D_NULL_ASSERT(mdl);
    ...
    static void printOpMat_(u32 idx, const NNSG3dResMdl * mdl) {
        NNS_G3D_NULL_ASSERT(mdl);
    ...
    static void printOpShp_(u32 idx, const NNSG3dResMdl * mdl) {
        NNS_G3D_NULL_ASSERT(mdl);
    ...
    static void printOpNodeDescFlag(u32 flag) {
        RES_PRINTF(", 0x%02x", flag);
    }

    void NNS_G3dPrintSbc(const u8 * pCode, u32 sizeByte, const NNSG3dResMdl * mdl) {
        u32 idx;
    ...
    void NNS_G3dPrintDL(const u32 * dl, u32 szByte) {
    ...
    #define NNS_G3D_DLPARAM_COLOR(rgb) \
        RES_PRINTF("(%d, %d, %d) ", (rgb) & 0x1f, ((rgb) >> 5) & 0x1f, ((rgb) >> 10) & 0x1f)
        ...
        while (cur < dl + (szByte >> 2)) {
            ...
            for (i = 0; i < 4; ++i) {
                ...
                case G3OP_COLOR:
                    ...
                    break;
                case G3OP_NORMAL:
                    RES_PRINTF("Normal (%f, %f, %f)\n", (((s32)((*param & 0x3ff) << 22) >> 19) / 4096.f), (((s32)((*param & 0xffc00) << 12) >> 19) / 4096.f), (((s32)((*param & 0x3ff00000) << 2) >> 19) / 4096.f));
                    param += G3OP_NORMAL_NPARAMS;
                    break;
                case G3OP_TEXCOORD:
                    RES_PRINTF("TexCoord (%f, %f)\n", (((s32)(*param << 16) >> 8) / 4096.f), (((s32)(*param & 0xffff0000) >> 8) / 4096.f));
                ...
                case G3OP_VTX_YZ:
                    ...
                    break;
                case G3OP_VTX_DIFF:
                    RES_PRINTF("VertexDiff (%f, %f, %f)\n", (((s32)((*param & 0x3ff) << 22) >> 22) / 4096.f), (((s32)((*param & 0xffc00) << 12) >> 22) / 4096.f), (((s32)((*param & 0x3ff00000) << 2) >> 22) / 4096.f));
                ...
                case G3OP_LIGHT_VECTOR:
                    RES_PRINTF("LightVector %d, (%f, %f, %f)\n", *param >> 30, ((s32)((*param & 0x3ff) << 22) >> 19) / 4096.f, ((s32)((*param & 0x3ff) << 12) >> 19) / 4096.f, ((s32)((*param & 0x3ff) << 2) >> 19) / 4096.f);
                ...
                case G3OP_SWAP_BUFFERS:
                    ...
                    break;
                case G3OP_VIEWPORT:
                    RES_PRINTF("ViewPort (%d, %d)-(%d, %d)", *param & 0xff, (*param >> 8) & 0xff, (*param >> 16) & 0xff, (*param >> 24) & 0xff);
    ...
    static const NNSG3dResName * getTexNameForMatID_(const NNSG3dResMat * mat, u32 idx) {
        u32 i, j;
        ...
        for (i = 0; i < texDict->numEntry; ++i) {
            const NNSG3dResDictTexToMatIdxData * data = (const NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByIdx(texDict, i);
    ...
    static const NNSG3dResName * getPlttNameForMatID_(const NNSG3dResMat * mat, u32 idx) {
        u32 i, j;
        ...
        for (i = 0; i < plttDict->numEntry; ++i) {
            const NNSG3dResDictTexToMatIdxData * data = (const NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByIdx(plttDict, i);
            u8 * first = (u8 *)mat + data->offset;
    ...
    static void printRGB_(const char * name, u32 rgb) {
        u32 r = rgb & 0x1f;
        u32 g = (rgb >> 5) & 0x1f;
        u32 b = (rgb >> 10) & 0x1f;
    ...
    void NNS_G3dPrintMatData(const NNSG3dResMat * mat, u32 idx) {
        const NNSG3dResMatData * matData = NNS_G3dGetMatDataByIdx(mat, idx);
        const NNSG3dResName * resName = NNS_G3dGetMatNameByIdx(mat, idx);
        const NNSG3dResDict * plttDict = (const NNSG3dResDict *)((u8 *)mat + mat->ofsDictPlttToMatList);
    ...
    void NNS_G3dPrintMat(const NNSG3dResMat * mat) {
        NNS_G3D_NULL_ASSERT(mat);
    ...
    void NNS_G3dPrintShp(const NNSG3dResShp * shp) {
    ...
    void NNS_G3dPrintMdl(const NNSG3dResName * resName, const NNSG3dResMdl * mdl) {
        NNS_G3D_NULL_ASSERT(resName);
        NNS_G3D_NULL_ASSERT(mdl);
    ...
    void NNS_G3dPrintMdlSet(const NNSG3dResMdlSet * mdlSet) {
    ...
    void NNS_G3dPrintNSBMD(const u8 * binFile) {
        u32 i;
    ...
    void NNS_G3dPrintNSBTX(const u8 * binFile) {
        u32 i;
        NNSG3dResFileHeader * header;
        u32 numBlocks;
        const u32 * blks;
        ...
        for (i = 0; i < numBlocks; ++i) {
            NNSG3dResDataBlockHeader * blk = (NNSG3dResDataBlockHeader *)((u8 *)header + blks[i]);
            u32 kind = blk->kind;
            u8 * contents = (u8 *)blk + sizeof(NNSG3dResDataBlockHeader);
    ...
    void NNS_G3dPrintBinFile(const u8 * binFile) {
        NNS_G3D_NULL_ASSERT(binFile);
    ...
    void chkDict_(const NNSG3dResDict * dict, const NNSG3dResName * name, u32 idx) {
