...
#include <nnsys/g3d/binres/res_struct_accessor.h>
    ...
    static void printSTData_(u32 info, u32 data) {
    ...
    static void printRData_(u32 info, u32 data) {
        if (info & NNS_G3D_TEXSRTANM_ELEM_CONST) {
    ...
    void NNS_G3dPrintTexSRTAnm(const NNSG3dResTexSRTAnm * pTexSRTAnm) {
        u8 j;
        tabPlus_();
        for (j = 0; j < pTexSRTAnm->dict.numEntry; j++) {
            ...
            pAnmData = (const NNSG3dResDictTexSRTAnmData *)NNS_G3dGetResDataByIdx(&pTexSRTAnm->dict, j);
    ...
    void NNS_G3dPrintTexSRTAnmSet(const NNSG3dResTexSRTAnmSet * pAnmSet) {
        u8 i;
    ...
    void NNS_G3dPrintNSBTA(const u8 * binFile) {
        u32 i;
