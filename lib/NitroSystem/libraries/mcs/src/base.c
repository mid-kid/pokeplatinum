...
static void DataRecvCallback(const void * pData, u32 dataSize, u32 userData, u32 offset, u32 totalSize) {
    ...
    if (NNS_McsGetRingBufferWritableBytes(cbInfo->ringBuf) < offset + dataSize) {
        NNS_WARNING(FALSE, "NNS mcs error : buffer is not enough. writable %d, data size %d, offset %d, total size %d\n", NNS_McsGetRingBufferWritableBytes(cbInfo->ringBuf), dataSize, offset, totalSize);
        return;
    }

    {
        BOOL ret = NNS_McsWriteRingBufferEx(cbInfo->ringBuf, pData, offset, dataSize, totalSize);
        NNS_ASSERT(ret);
    }
}
...
static NNS_MCS_INLINE BOOL CloseEmulator() {
    NNSiMcsEnsWork * pWork = GetEnsWorkPtr();
...
void NNS_McsRegisterStreamRecvBuffer(u16 channel, void * buf, u32 bufSize) {
    ...
    NNS_McsRegisterRecvCallback(&pInfo->cbInfo, channel, DataRecvCallback, (uintptr_t)pInfo);
    Unlock();
}
    ...
    while (offset < size) {
        ...
        if (restSize > length && length < NNSi_MCS_MSG_DATA_SIZE_MIN) {
            ...
            if (OS_IsRunOnEmulator()) {
                ...
                if (!NNS_McsWriteRingBuffer(pWork->wrRb, &pWork->msgBuf, offsetof(NNSiMcsEnsMsgBuf, buf) + NNSi_MCS_MSG_HEAD_SIZE + length)) {
                    NNS_WARNING(FALSE, "NNS Mcs error: send error\n");
                    break;
                }
            } else {
                if (!NITROToolAPIWriteStream(channel, pBlock, NNSi_MCS_MSG_HEAD_SIZE + length)) {
