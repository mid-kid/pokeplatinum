...
static void ApplyCurrentAnimResult_(NNSG2dCellAnimation * pCellAnim) {
    ...
    {
        const NNSG2dAnimationElement elemType = NNSi_G2dGetAnimSequenceElementType(pCellAnim->animCtrl.pAnimSequence->animType);
    ...
    if (NNS_G2dCellDataBankHasVramTransferData(pCellBank) && NNSi_G2dIsCellAnimVramTransferHandleValid(pCellAnim)) {
        const NNSG2dCellVramTransferData * pCellTransferData = NNSi_G2dGetCellVramTransferData(pCellBank, pAnimResult->index);
        NNS_G2dSetCellTransferStateRequested(pCellAnim->cellTransferStateHandle, pCellTransferData->srcDataOffset, pCellTransferData->szByte);
    }
}
...
void NNS_G2dInitCellAnimation(NNSG2dCellAnimation * pCellAnim, const NNSG2dAnimSequence * pAnimSeq, const NNSG2dCellDataBank * pCellDataBank) {
    ...
    InitCellAnimationImpl_(pCellAnim, pAnimSeq, pCellDataBank, NNS_G2D_INVALID_CELL_TRANSFER_STATE_HANDLE);
}
...
void NNS_G2dInitCellAnimationVramTransfered(NNSG2dCellAnimation * pCellAnim, const NNSG2dAnimSequence * pAnimSeq, const NNSG2dCellDataBank * pCellBank, u32 vramStateHandle, u32 dstAddr3D, u32 dstAddr2DMain, u32 dstAddr2DSub, const void * pSrcNCGR, const void * pSrcNCBR, u32 szSrcData) {
    ...
    {
        const NNSG2dVramTransferData * pVramData = (const NNSG2dVramTransferData *)pCellBank->pVramTransferData;

        NNSi_G2dInitCellTransferState(vramStateHandle, dstAddr3D, dstAddr2DMain, dstAddr2DSub, pVramData->szByteMax, pSrcNCGR, pSrcNCBR, szSrcData);
...
void NNS_G2dSetCellAnimationSequence(NNSG2dCellAnimation * pCellAnim, const NNSG2dAnimSequence * pAnimSeq) {
    NNS_G2D_NULL_ASSERT(pCellAnim);
    NNS_G2D_NULL_ASSERT(pAnimSeq);
    NNS_G2D_ASSERTMSG(NNS_G2dGetAnimSequenceAnimType(pAnimSeq) == NNS_G2D_ANIMATIONTYPE_CELL, "A cell-Animation's Data is expected");
...
u16 NNS_G2dMakeCellToOams(GXOamAttr * pDstOams, u16 numDstOam, const NNSG2dCellData * pCell, const MtxFx22 * pMtxSR, const NNSG2dFVec2 * pBaseTrans, u16 affineIndex, BOOL bDoubleAffine) {
    ...
    const u16 numOBJ = (numDstOam < pCell->numOAMAttrs) ? numDstOam : pCell->numOAMAttrs;
    ...
    for (i = 0; i < numOBJ; i++) {
        ...
        if (pMtxSR != NULL || pBaseTrans != NULL) {
            ...
            if (pMtxSR != NULL) {
                ...
                NNSi_G2dRemovePositionAdjustmentFromDoubleAffineOBJ(pDstOam, &objTrans);

                {
                    const GXOamEffect effectTypeAfter = (bDoubleAffine) ? GX_OAM_EFFECT_AFFINE_DOUBLE : GX_OAM_EFFECT_AFFINE;
                    ...
                    NNSi_G2dAdjustDifferenceOfRotateOrientation(pDstOam, pMtxSR, &objTrans, bShouldAdjust);
                }
            }
            ...
            G2_SetOBJPosition(pDstOam, (objTrans.x + 0x800) >> FX32_SHIFT, (objTrans.y + 0x800) >> FX32_SHIFT);
        }
    }
