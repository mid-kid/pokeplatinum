...
static void DrawSprite_(const NNSG2dSimpleSprite * pS, const NNSG2dBasicSprite * pB, const NNSG2dExtendedSprite * pE) {
    ...
    {
        G3_Translate((pS->pos.x << FX32_SHIFT) + ((pS->size.x >> 1) << FX32_SHIFT), (pS->pos.y << FX32_SHIFT) + ((pS->size.y >> 1) << FX32_SHIFT), ((int)pS->priority << FX32_SHIFT));

        if (NNS_G2dIsSpriteAttrEnable(NNS_G2D_SPRITEATTR_ROTO)) {
            G3_Translate(pE->rotO.x << FX32_SHIFT, pE->rotO.y << FX32_SHIFT, 0);
        }
        ...
        G3_Scale(pS->size.x << FX32_SHIFT, pS->size.y << FX32_SHIFT, FX32_ONE);

        if (NNS_G2dIsSpriteAttrEnable(NNS_G2D_SPRITEATTR_TEXTURE)) {
            if (pB->pTextureAttr) {
                const NNSG2dImageAttr * pAttr = pB->pTextureAttr;

                G3_TexImageParam(pAttr->fmt, GX_TEXGEN_TEXCOORD, pAttr->sizeS, pAttr->sizeT, GX_TEXREPEAT_NONE, GX_TEXFLIP_NONE, pAttr->plttUse, pB->texAddr);
        ...
        if (NNS_G2dIsSpriteAttrEnable(NNS_G2D_SPRITEATTR_ALPHA)) {
            ...
            G3_PolygonAttr(GX_LIGHTMASK_NONE, GX_POLYGONMODE_MODULATE, GX_CULL_NONE, currentPolygonID_, pS->alpha, 0);
        }
