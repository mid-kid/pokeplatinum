...
BOOL NNS_G2dGetUnpackedPaletteData(void * pNclrFile, NNSG2dPaletteData ** ppPltData) {
...

    {
        ...
        NNS_G2D_ASSERTMSG(pBinFile->signature == NNS_G2D_BINFILE_SIG_PALETTEDATA ||
                          pBinFile->signature == NNS_G2D_BINFILE_SIG_PALETTEDATA_OLD, "Input file signature is invalid for this method.");
    }

    NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileVersionValid(pNclrFile, BIN_FILE_VERSION(NCLR)), "Input file is obsolete. Please use the new g2dcvtr.exe.");

    {
        NNSG2dBinaryFileHeader * pBinFile = (NNSG2dBinaryFileHeader *)pNclrFile;

        {
            NNSG2dPaletteDataBlock * pBinBlk = (NNSG2dPaletteDataBlock *)NNS_G2dFindBinaryBlock(pBinFile, NNS_G2D_BINBLK_SIG_PALETTEDATA);
...
BOOL NNS_G2dGetUnpackedPaletteCompressInfo(void * pNclrFile, NNSG2dPaletteCompressInfo ** ppPltCmpInfo) {
...

    {
        NNSG2dBinaryFileHeader * pBinFile = (NNSG2dBinaryFileHeader *)pNclrFile;
        {
            NNSG2dPaletteCompressDataBlock * pBinBlk = (NNSG2dPaletteCompressDataBlock *)NNS_G2dFindBinaryBlock(pBinFile, NNS_G2D_BINBLK_SIG_PALETTECOMPINFO);
