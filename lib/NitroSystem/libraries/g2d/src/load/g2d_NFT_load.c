...
#else
    NNS_G2D_ASSERTMSG(NNS_G2dIsBinFileValid(pNftrFile, NNS_G2D_BINFILE_SIG_FONTDATA, BIN_FILE_VERSION(NFTR)), "Input file is obsolete. Please use latest fontcvtr.");
#endif
...
void NNSi_G2dUnpackNFT(NNSG2dBinaryFileHeader * pHeader) {
    ...
    static void PrintFontCMapScanList(const NNSG2dCMapInfoScan * scan) {
        ...
        for (i = 0; i < scan->num; i++) {
            OS_Printf("   %4d: 0x%04X -> %d\n", i, scan->entries[i].ccode, scan->entries[i].index);
        }
    }
