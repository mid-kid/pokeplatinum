...
BOOL NNS_G2dGetUnpackedMultiCellBank(void * pNmcrFile, NNSG2dMultiCellDataBank ** ppMCBank) {
    ...
    NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileSignatureValid(pNmcrFile, NNS_G2D_BINFILE_SIG_MULTICELL), "Input file signature is invalid for this method.");
    NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileVersionValid(pNmcrFile, BIN_FILE_VERSION(NMCR)), "Input file is obsolete. Please use the new g2dcvtr.exe.");

    {
        NNSG2dBinaryFileHeader * pBinFile = (NNSG2dBinaryFileHeader *)pNmcrFile;
        
        {
            NNSG2dMultiCellDataBankBlock * pBinBlk = (NNSG2dMultiCellDataBankBlock *)NNS_G2dFindBinaryBlock(pBinFile, NNS_G2D_BLKSIG_MULTICELLBANK);
            if (pBinBlk) {
...
void NNS_G2dUnpackNMC(NNSG2dMultiCellDataBank * pMCellData) {
    ...
    {
        pMCellData->pMultiCellDataArray = NNS_G2D_UNPACK_OFFSET_PTR(pMCellData->pMultiCellDataArray, pMCellData);
        pMCellData->pHierarchyDataArray = NNS_G2D_UNPACK_OFFSET_PTR(pMCellData->pHierarchyDataArray, pMCellData);

        {
            ...
            for (i = 0; i < pMCellData->numMultiCellData; i++) {
                pCell[i].pHierDataArray = NNS_G2D_UNPACK_OFFSET_PTR(pCell[i].pHierDataArray, pMCellData->pHierarchyDataArray);
            }
        }

        if (pMCellData->pExtendedData != NULL) {
            pMCellData->pExtendedData = NNS_G2D_UNPACK_OFFSET_PTR(pMCellData->pExtendedData, pMCellData);
