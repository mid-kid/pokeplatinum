...
BOOL NNS_G2dGetUnpackedEntityBank(void * pNenrFile, NNSG2dEntityDataBank ** ppEntityBank) {
    ...
    NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileSignatureValid(pNenrFile, NNS_G2D_BINFILE_SIG_ENTITY), "Input file signature is invalid for this method.");
    NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileVersionValid(pNenrFile, BIN_FILE_VERSION(NENR)), "Input file is obsolete. Please use the new g2dcvtr.exe.");
    
    {
        NNSG2dBinaryFileHeader * pBinFile = (NNSG2dBinaryFileHeader *)pNenrFile;
        {
            NNSG2dEntityDataBankBlock * pBinBlk = (NNSG2dEntityDataBankBlock *)NNS_G2dFindBinaryBlock(pBinFile, NNS_G2D_BLKSIG_ENTITYBANK);
...
void NNS_G2dUnpackNEN(NNSG2dEntityDataBank * pDataBank) {
    ...
    {
        pDataBank->pEntityDataArray = NNS_G2D_UNPACK_OFFSET_PTR(pDataBank->pEntityDataArray, pDataBank);
        pDataBank->pAnimSequenceIdxArray = NNS_G2D_UNPACK_OFFSET_PTR(pDataBank->pAnimSequenceIdxArray, pDataBank);

        {
            ...
            for (i = 0; i < pDataBank->numEntityDatas; i++) {
                pEntity[i].animData.pAnimSequenceIdxArray =
                    NNS_G2D_UNPACK_OFFSET_PTR(pEntity[i].animData.pAnimSequenceIdxArray, pDataBank->pAnimSequenceIdxArray);
            }
        }
    }
