...
static NNS_G2D_INLINE BOOL IsOamEntryFuncsValid_(const NNSG2dOamManagerInstanceEx * pMan, const NNSG2dOamExEntryFunctions * pF) {
    const BOOL bValid = (BOOL)((pF != NULL) && (pF->getOamCapacity != NULL) && (pF->entryNewOam != NULL));

    if (pMan->pAffineBuffer != NULL || pMan->lengthAffineBuffer != 0) {
        return (BOOL)(bValid && (pF->getAffineCapacity != NULL) && (pF->entryNewAffine != NULL));
    } else {
        return bValid;
    }
}
    ...
    while (numTotalOamDrawn < pMan->numUsedOam) {
        ...
        if (pOamList->numChunks != 0) {
            ...
            if (pOamList->numChunks <= currentCapacity) {
                ...
                pOamList->numLastFrameDrawn = (u16)((pOamList->numDrawn + pOamList->numLastFrameDrawn) % pOamList->numChunks);
...
BOOL NNS_G2dEntryOamManExOam(NNSG2dOamManagerInstanceEx * pMan, const GXOamAttr * pOam, u8 priority) {
...

    {
        ...
        return EntryNewOamWithAffine_(pOamList, pOamChunk, NNS_G2D_OAM_AFFINE_IDX_NONE, pMan->drawOrderType);
    }
}
...
BOOL NNS_G2dEntryOamManExOamWithAffineIdx(NNSG2dOamManagerInstanceEx * pMan, const GXOamAttr * pOam, u8 priority, u16 affineIdx) {
...

    {
        ...
        return EntryNewOamWithAffine_(pOamList, pOamChunk, affineIdx, pMan->drawOrderType);
    }
}
...

            {
                ...
                for (i = 0; i < pMan->lengthOfOrderingTbl; i++) {
                    ...
                    if (pOamList->bDrawn) {
                        ...
                        numTotalOamDrawn = DrawOamChunks_(pMan, pOamList, pChunk, pOamList->numDrawn, capacityOfHW, numTotalOamDrawn);
                    }
                }
            }
        }
