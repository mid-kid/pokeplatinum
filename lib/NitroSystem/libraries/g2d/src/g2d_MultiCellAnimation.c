    ...
    if (pMCInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
        ...
        for (i = 0; i < pMcData->numNodes; i++) {
            ...
            if (!pCellAnimArray[cellAnimIdx].bInited) {
                ...
                SetNodeDataToCellAnim_(pCellAnim, pNodeData, pMCInst->pAnimDataBank, mcTotalFrame);
...
static NNS_G2D_INLINE void ApplyCurrentAnimResult_(NNSG2dMultiCellAnimation * pMultiCellAnim) {
    ...
    pData = NNS_G2dGetMultiCellDataByIdx(pMultiCellAnim->pMultiCellDataBank, pAnimResult->index);
    ...
    SetMCDataToMCInstanceImpl_(&pMultiCellAnim->multiCellInstance, pData, pMultiCellAnim->totalVideoFrame);
...
static u16 MakeCellAnimToOams_(GXOamAttr * pDstOams, u16 numDstOams, const NNSG2dCellAnimation * pCellAnim, const NNSG2dSVec2 * pNodeTrans, const MtxFx22 * pMtxSR, const NNSG2dFVec2 * pBaseTrans, u16 affineIndex, BOOL bDoubleAffine) {
    ...
    {
        AddSVec_(&pContentsSRT->srtData.trans, pNodeTrans, &vTransS);
    ...
    return NNS_G2dMakeCellToOams(pDstOams, numDstOams, NNS_G2dGetCellAnimationCurrentCell(pCellAnim), pMtxSR, &vTransF, affineIndex, bDoubleAffine);
}
    ...
    for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
        ...
        if (numRestOams > 0) {
            ...
            NNS_G2D_WARNING(pNode->type == NNS_G2D_NODETYPE_CELL &&
                            !NNSi_G2dSrtcIsAffineEnable_SR(&pNode->srtCtrl), "A SR-Transformation of a multicell-node was ignored.");

            numOamUsed = MakeCellAnimToOams_(pDstOams, (u16)numRestOams, pNode[i].pContent, &pNode[i].srtCtrl.srtData.trans, pMtxSR, pBaseTrans, affineIndex, bDoubleAffine);
...
static NNS_G2D_INLINE u16 MakeSimpleMultiCellToOams_ShareCellAnims_(GXOamAttr * pDstOams, u16 numDstOams, const NNSG2dMultiCellInstance * pMCellInst, const MtxFx22 * pMtxSR, const NNSG2dFVec2 * pBaseTrans, u16 affineIndex, BOOL bDoubleAffine) {
    ...
    for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
        const NNSG2dMultiCellHierarchyData * pNode = &pMCellInst->pCurrentMultiCell->pHierDataArray[i];
        ...
        if (numRestOams > 0) {
            ...
            numOamUsed = MakeCellAnimToOams_(pDstOams, (u16)numRestOams, &pMCCellAnimArray[cellAnimIdx].cellAnim, &nodeTrans, pMtxSR, pBaseTrans, affineIndex, bDoubleAffine);
            numRestOams -= numOamUsed;
            pDstOams += numOamUsed;
        } else {
            break;
...
static NNS_G2D_INLINE void InitMCInstance_(NNSG2dMultiCellInstance * pMultiCell, NNSG2dNode * pNodeArray, NNSG2dCellAnimation * pCellAnim, u16 numNode, const NNSG2dCellAnimBankData * pAnimBank, const NNSG2dCellDataBank * pCellDataBank) {
...

    {
        ...
        for (i = 0; i < numNode; i++) {
            ...
            NNS_G2dInitCellAnimation(&pCellAnim[i], NNS_G2dGetAnimSequenceByIdx(pAnimBank, 0), pCellDataBank);
        }
    }
}
...
#pragma unused( pMultiCellDataBank )
    ...
    InitMCInstance_(&pMultiCellAnim->multiCellInstance, pNodeArray, pCellAnim, numNode, pAnimBank, pCellDataBank);
...
static NNS_G2D_INLINE void InitMCAnimation_SharingCellAnim_(NNSG2dMultiCellAnimation * pMultiCellAnim, void * pWork, const NNSG2dCellAnimBankData * pAnimBank, const NNSG2dCellDataBank * pCellDataBank, const NNSG2dMultiCellDataBank * pMultiCellDataBank) {
...

    {
        ...
        {
            u16 i;
            const u16 numCellAnim = GetMCBankNumCellAnimRequired_(pMultiCellDataBank);
            NNSG2dMCCellAnimation * pMCCellAnim = (NNSG2dMCCellAnimation *)pMCInst->pCellAnimInstasnces;

            for (i = 0; i < numCellAnim; i++) {
                NNS_G2dInitCellAnimation(&pMCCellAnim[i].cellAnim, NNS_G2dGetAnimSequenceByIdx(pAnimBank, 0), pCellDataBank);
...
void NNS_G2dSetAnimSequenceToMCAnimation(NNSG2dMultiCellAnimation * pMultiCellAnim, const NNSG2dAnimSequence * pAnimSeq) {
    ...
    NNS_G2D_ASSERTMSG(NNS_G2dGetAnimSequenceAnimType(pAnimSeq) == NNS_G2D_ANIMATIONTYPE_MULTICELLLOCATION, "NNSG2dAnimationType must be MultiCellLocation.");
...
void NNS_G2dInitMCAnimationInstance(NNSG2dMultiCellAnimation * pMultiCellAnim, void * pWork, const NNSG2dCellAnimBankData * pAnimBank, const NNSG2dCellDataBank * pCellDataBank, const NNSG2dMultiCellDataBank * pMultiCellDataBank, NNSG2dMCType mcType) {
    ...
    if (mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
        InitMCAnimation_SharingCellAnim_(pMultiCellAnim, pWork, pAnimBank, pCellDataBank, pMultiCellDataBank);
    } else {
        const u16 numNode = NNS_G2dGetMCBankNumNodesRequired(pMultiCellDataBank);
        NNSG2dNode * pNodeArray = pWork;
        NNSG2dCellAnimation * pCellAnimArray = (NNSG2dCellAnimation *)(pNodeArray + numNode);

        InitMCAnimation_(pMultiCellAnim, pNodeArray, pCellAnimArray, numNode, pAnimBank, pCellDataBank, pMultiCellDataBank);
    }
...
u16 NNS_G2dGetMCNumNodesRequired(const NNSG2dMultiCellAnimSequence * pMultiCellSeq, const NNSG2dMultiCellDataBank * pMultiCellDataBank) {
    ...
    NNS_G2D_NULL_ASSERT(pMultiCellDataBank);
    NNS_G2D_ASSERTMSG(NNS_G2dGetAnimSequenceAnimType(pMultiCellSeq) == NNS_G2D_ANIMATIONTYPE_MULTICELLLOCATION, "NNSG2dAnimationType must be MultiCellLocation.");

    {
        ...
        for (i = 0; i < pMultiCellSeq->numFrames; i++) {
            pAnmFrm = (const NNSG2dAnimDataSRT *)pMultiCellSeq->pAnmFrameArray[i].pContent;
            pMCell = NNS_G2dGetMultiCellDataByIdx(pMultiCellDataBank, pAnmFrm->index);
...
u32 NNS_G2dGetMCWorkAreaSize(const NNSG2dMultiCellDataBank * pMultiCellDataBank, NNSG2dMCType mcType) {
    if (mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
        return sizeof(NNSG2dMCCellAnimation) * GetMCBankNumCellAnimRequired_(pMultiCellDataBank);
    } else {
        return (sizeof(NNSG2dNode) + sizeof(NNSG2dCellAnimation)) * NNS_G2dGetMCBankNumNodesRequired(pMultiCellDataBank);
    }
}
...
void NNS_G2dRestartMCAnimation(NNSG2dMultiCellAnimation * pMultiCellAnim) {
    ...
    NNS_G2dSetAnimSequenceToMCAnimation(pMultiCellAnim, NNS_G2dGetAnimCtrlCurrentAnimSequence(pMCAnmCtrl));
...
u16 NNS_G2dMakeSimpleMultiCellToOams(GXOamAttr * pDstOams, u16 numDstOams, const NNSG2dMultiCellInstance * pMCellInst, const MtxFx22 * pMtxSR, const NNSG2dFVec2 * pBaseTrans, u16 affineIndex, BOOL bDoubleAffine) {
    if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
        return MakeSimpleMultiCellToOams_ShareCellAnims_(pDstOams, numDstOams, pMCellInst, pMtxSR, pBaseTrans, affineIndex, bDoubleAffine);
    } else {
        return MakeSimpleMultiCellToOams_(pDstOams, numDstOams, pMCellInst, pMtxSR, pBaseTrans, affineIndex, bDoubleAffine);
    }
}
...
void NNS_G2dTraverseMCNodes(NNSG2dMultiCellInstance * pMCellInst, NNSG2dMCTraverseNodeCallBack pCBFunc, u32 userParamater) {
...

    {
        ...
        if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
            NNSG2dMCCellAnimation * pMCCellAnimArray = (NNSG2dMCCellAnimation *)pMCellInst->pCellAnimInstasnces;

            for (i = 0; i < numNode; i++) {
                const NNSG2dMultiCellHierarchyData * pNodeData = &pMCData->pHierDataArray[i];
                const u16 cellAnimIdx = NNSi_G2dGetMC2NodeCellAinmIdx(pNodeData);

                bContinue = (*pCBFunc)(userParamater, pNodeData, &pMCCellAnimArray[cellAnimIdx].cellAnim, i);
            ...
            const NNSG2dNode * pNode = (const NNSG2dNode *)pMCellInst->pCellAnimInstasnces;

            for (i = 0; i < numNode; i++) {
                const NNSG2dMultiCellHierarchyData * pNodeData = &pMCData->pHierDataArray[i];
                bContinue = (*pCBFunc)(userParamater, pNodeData, pNode[i].pContent, i);
...
void NNS_G2dInitMCAnimation(NNSG2dMultiCellAnimation * pMultiCellAnim, NNSG2dNode * pNodeArray, NNSG2dCellAnimation * pCellAnim, u16 numNode, const NNSG2dCellAnimBankData * pAnimBank, const NNSG2dCellDataBank * pCellDataBank, const NNSG2dMultiCellDataBank * pMultiCellDataBank) {
    InitMCAnimation_(pMultiCellAnim, pNodeArray, pCellAnim, numNode, pAnimBank, pCellDataBank, pMultiCellDataBank);
}
...
void NNS_G2dInitMCInstance(NNSG2dMultiCellInstance * pMultiCell, NNSG2dNode * pNodeArray, NNSG2dCellAnimation * pCellAnim, u16 numNode, const NNSG2dCellAnimBankData * pAnimBank, const NNSG2dCellDataBank * pCellDataBank) {
    InitMCInstance_(pMultiCell, pNodeArray, pCellAnim, numNode, pAnimBank, pCellDataBank);
}
