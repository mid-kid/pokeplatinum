...
static NNS_G2D_INLINE GXOamAttr * GetOamCurrentPtr_(NNSG2dOamManagerInstance * pMan) {
    ...
    NNS_G2D_ASSERT(pMan->managedAttrArea.toIdx < NUM_HW_OAM_ATTR);
    {
        ...
        GXOamAttr * pret = GetOamBufferPtr_(pMan->type, pMan->managedAttrArea.currentIdx);
...
static NNS_G2D_INLINE GXOamAttr * GetOamFromPtr_(NNSG2dOamManagerInstance * pMan) {
    NNS_G2D_NULL_ASSERT(pMan);
    {
        GXOamAttr * pFrom = GetOamBufferPtr_(pMan->type, pMan->managedAttrArea.fromIdx);
...
static NNS_G2D_INLINE void ClearOamByDefaultValue_(NNSG2dOamType type) {
    ASSERT_OAMTYPE_VALID(type);
    MI_CpuFill16(GetOamBufferPtr_(type, 0), OAM_SETTING_INVISIBLE, GX_OAMATTR_SIZE * NUM_HW_OAM_ATTR);
}

static NNS_G2D_INLINE void SetOamReservationTblNotUsed_(NNSG2dOamType type) {
    ASSERT_OAMTYPE_VALID(type);
    DoRestoreArea_(GetOamReservationTblPtr_(type), 0, NUM_HW_OAM_ATTR - 1);
}

static NNS_G2D_INLINE void SetAffineReservationTblNotUsed_(NNSG2dOamType type) {
    ASSERT_OAMTYPE_VALID(type);
    DoRestoreArea_(GetAffineReservationTblPtr_(type), 0, NUM_HW_OAM_AFFINE - 1);
}
...
void NNS_G2dSetOamManagerAffine(NNSG2dOamType type, const MtxFx22 * mtx, u16 idx) {
    ...
    SDK_WARNING(IsOamNotUsed_(GetAffineReservationTblPtr_(type), idx, idx), "An invalid affine param setting to the managed area is detected. index = %d", idx);
...
void NNS_G2dApplyOamManagerToHW(NNSG2dOamManagerInstance * pMan) {
    NNS_G2D_NULL_ASSERT(pMan);
    NNS_G2D_ASSERTMSG(pMan->type != NNS_G2D_OAMTYPE_SOFTWAREEMULATION, " For the NNS_G2D_OAMTYPE_SOFTWAREEMULATION type Manager, Use NNS_G2dApplyOamManagerToHWSprite() instead.");
    {
        if (pMan->bFastTransferEnable) {
            LoadOamAndAffineFast_(pMan->type, pMan->managedAttrArea.fromIdx, pMan->managedAttrArea.toIdx);
        } else {
            LoadOam_(pMan->type, pMan->managedAttrArea.fromIdx, pMan->managedAttrArea.toIdx);
            if (IsManageAreaValid_(&pMan->managedAffineArea)) {
                LoadAffine_(pMan->type, pMan->managedAffineArea.fromIdx, pMan->managedAffineArea.toIdx);
            }
        }
    }
}
...
void NNS_G2dApplyOamManagerToHWSprite(NNSG2dOamManagerInstance * pMan, const NNSG2dImageAttr * pTexImageAttr, u32 texBaseAddr, u32 pltBaseAddr) {
    NNS_G2D_NULL_ASSERT(pMan);
    NNS_G2D_NULL_ASSERT(pTexImageAttr);
    NNS_G2D_ASSERTMSG(pMan->type == NNS_G2D_OAMTYPE_SOFTWAREEMULATION, " For the NNS_G2D_OAMTYPE_MAIN SUB type Manager, Use NNS_G2dApplyOamManagerToHW() instead.");
    ...
    if (pMan->spriteZoffsetStep != 0) {
...

        {
            ...
            DrawBy3DGraphicsEngine_(pFrom, numOam, pTexImageAttr, texBaseAddr, pltBaseAddr);
        }
...
void NNS_G2dResetOamManagerBuffer(NNSG2dOamManagerInstance * pMan) {
    NNS_G2D_NULL_ASSERT(pMan);
    {
        if (pMan->bFastTransferEnable) {
            ...
            ResetOam_(pMan->type, pMan->managedAttrArea.fromIdx, pMan->managedAttrArea.toIdx);
        }
    }
