...
static void * AllocFromHead(NNSiFndHeapHead * pHeapHd, u32 size, int alignment) {
    ...
    return AllocUsedBlockFromFreeBlock(
        pExpHeapHd, pMBlkHdFound, foundMBlock, size, NNS_FND_EXPHEAP_ALLOC_DIR_FRONT);
}
...
static void * AllocFromTail(NNSiFndHeapHead * pHeapHd, u32 size, int alignment) {
    ...
    return AllocUsedBlockFromFreeBlock(
        pExpHeapHd, pMBlkHdFound, foundMBlock, size, NNS_FND_EXPHEAP_ALLOC_DIR_REAR);
}
...

        {
            ...
            if (pExpHeapHd->mbFreeList.head == NULL) {
                ...
                for (pMBHead = pExpHeapHd->mbFreeList.head; pMBHead; pMBHead = pMBHead->pMBHeadNext) {
                    ...
                    OS_Printf("    %s %08x: %8d  %3d %3d  (%08x %08x)\n", " free", GetMemPtrForMBlock(pMBHead), pMBHead->blockSize, GetGroupIDForMBlock(pMBHead), GetAlignmentForMBlock(pMBHead), pMBHead->pMBHeadPrev ? GetMemPtrForMBlock(pMBHead->pMBHeadPrev) : NULL, pMBHead->pMBHeadNext ? GetMemPtrForMBlock(pMBHead->pMBHeadNext) : NULL);
