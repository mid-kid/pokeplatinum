...
BOOL NNS_SndPlayerCreateHeap(int playerNo, NNSSndHeapHandle heap, u32 size) {
    ...
    playerHeapHandle = NNS_SndHeapCreate((u8 *)buffer + sizeof(NNSSndPlayerHeap), size);
...
void NNS_SndPlayerPauseByPlayerNo(int playerNo, BOOL flag) {
    ...
    for (seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPlayer[ playerNo ].playerList, NULL); seqPlayer != NULL; seqPlayer = next) {
...
void NNS_SndPlayerPauseAll(BOOL flag) {
    ...
    for (seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPrioList, NULL); seqPlayer != NULL; seqPlayer = next) {
...
void NNS_SndPlayerSetTrackMute(NNSSndHandle * handle, u16 trackBitMask, BOOL flag) {
    ...
    SND_SetTrackMute(handle->player->playerNo, trackBitMask, flag);
}
    ...
    void NNS_SndPlayerSetTrackMuteEx(NNSSndHandle * handle, u16 trackBitMask, NNSSndSeqMute mute) {
        ...
        SND_SetTrackMuteEx(handle->player->playerNo, trackBitMask, (SNDSeqMute)mute);
    }
...
void NNS_SndPlayerSetTrackVolume(NNSSndHandle * handle, u16 trackBitMask, int volume) {
    ...
    SND_SetTrackVolume(
        handle->player->playerNo, trackBitMask, SND_CalcDecibel(volume));
}
...
void NNSi_SndPlayerMain(void) {
    ...
    for (seqPlayer = (NNSSndSeqPlayer *)NNS_FndGetNextListObject(&sPrioList, NULL); seqPlayer != NULL; seqPlayer = next) {
        ...
        fader = SND_CalcDecibel(seqPlayer->initVolume)
...
void NNSi_SndPlayerStartSeq(NNSSndSeqPlayer * seqPlayer, const void * seqDataBase, u32 seqDataOffset, const SNDBankData * bank) {
    ...
    SND_PrepareSeq(seqPlayer->playerNo, seqDataBase, seqDataOffset, bank);
    
    if (player->allocChBitFlag) {
        SND_SetTrackAllocatableChannel(seqPlayer->playerNo, 0xffff, player->allocChBitFlag);
    }
