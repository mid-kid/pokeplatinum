...
static NNSGfdFrmPlttVramManager s_managerState_ = {
    0, 0, 0
};
...
static void FrmPlttVramDebugDumpCallBack_(u32 loAddr, u32 hiAddr, u32 szFree, u32 szTotal) {
    ...
    OS_Printf("    %08d / %08d bytes (%6.2f%%) used \n", szTotal - szFree, szTotal, 100.0f * (szTotal - szFree) / szTotal);
    OS_Printf("==========================================\n");
}
...
void NNS_GfdDumpFrmPlttVramManagerEx(NNSGfdFrmPlttVramDebugDumpCallBack pFunc) {
    ...
    (*pFunc)(s_managerState_.loAddr, s_managerState_.hiAddr, s_managerState_.hiAddr - s_managerState_.loAddr, s_managerState_.szTotal);
}
