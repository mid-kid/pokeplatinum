...
static NNS_GFD_INLINE NNSGfdFrmTexRegionState * Get4x4IdxRegion_(const NNSGfdFrmTexRegionState * pRegion) {
    ...
    default:
        NNS_GFD_WARNING("Invalid region is detected. in Get4x4IdxRegion_(). ");
        break;
    }
...
static void DefaultDebugDumpFunc_(int index, u32 startAddr, u32 endAddr, u32 blockMax, BOOL bActive, void * pUserContext) {
    ...
    if (bActive) {
        OS_Printf("%02d    : 0x%08x  : 0x%08x  : 0x%08x  \n", index, startAddr, endAddr, endAddr - startAddr);
...
void NNS_GfdDumpFrmTexVramManager() {
    ...
    NNSiGfdDefaultDebugContext context = {
        0, 0
    };
    ...
    OS_Printf("    %08d / %08d bytes (%6.2f%%) used \n", context.szTotalReserved - context.szTotalFree, context.szTotalReserved, 100.0f * (context.szTotalReserved - context.szTotalFree) / context.szTotalReserved);
    OS_Printf("==========================================\n");
}
...
void NNS_GfdDumpFrmTexVramManagerEx(NNSGfdFrmTexVramDebugDumpCallBack pFunc, void * pUserContext) {
    ...
    for (i = 0; i < NNS_GFD_NUM_TEX_VRAM_REGION; i++) {
        ...
        (*pFunc)(i, pState->head + pState->baseAddress, pState->tail + pState->baseAddress, (pState->bHalfSize) ? (u32)(NNS_GFD_TEX_SLOT_SIZE / 2) : NNS_GFD_TEX_SLOT_SIZE, pState->bActive, pUserContext);
    }
}
