...
#pragma unused(opt)
    ...
    if (b4Pltt) {
        result = NNSi_GfdAllocLnkVramAligned(&mgr_.mgr, &mgr_.pBlockPoolList, &addr, szByte, 0x08);

        if (addr + szByte > NNS_GFD_4PLTT_MAX_ADDR) {
            if (!NNSi_GfdFreeLnkVram(&mgr_.mgr, &mgr_.pBlockPoolList, addr, szByte)) {
            }

            return NNS_GFD_ALLOC_ERROR_PLTTKEY;
        }
    } else {
        result = NNSi_GfdAllocLnkVramAligned(&mgr_.mgr, &mgr_.pBlockPoolList, &addr, szByte, 0x10);
    }
...
int NNS_GfdFreeLnkPlttVram(NNSGfdPlttKey plttKey) {
    ...
    const BOOL result = NNSi_GfdFreeLnkVram(&mgr_.mgr, &mgr_.pBlockPoolList, addr, szByte);
...
void NNS_GfdResetLnkPlttVramState(void) {
    mgr_.pBlockPoolList
        = NNSi_GfdInitLnkVramBlockPool(
              (NNSiGfdLnkVramBlock *)mgr_.pWorkHead, mgr_.szByteManagementWork / sizeof(NNSiGfdLnkVramBlock));

    {
        BOOL result;
        NNSi_GfdInitLnkVramMan(&mgr_.mgr);

        result = NNSi_GfdAddNewFreeBlock(&mgr_.mgr, &mgr_.pBlockPoolList, 0, mgr_.szByte);
